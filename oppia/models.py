...
class Activity(models.Model):
    ...
    def get_next_activity(self):
        try:
            next_activity = Activity.objects.get(section__course=self.section.course, order=self.order + 1,
                                                 section=self.section)
        except Activity.DoesNotExist:
            try:
                next_activity = Activity.objects.get(section__course=self.section.course,
                                                     section__order=self.section.order + 1, order=1)
            except Activity.DoesNotExist:
                next_activity = None
    ...
    def get_previous_activity(self):
        try:
            prev_activity = Activity.objects.get(section__course=self.section.course, order=self.order - 1,
                                                 section=self.section)
        except Activity.DoesNotExist:
            try:
                max_order = Activity.objects.filter(section__course=self.section.course,
                                                    section__order=self.section.order - 1).aggregate(
                    max_order=Max('order'))
...
class Tracker(models.Model):
    ...
    def is_first_tracker_today(self):
        ...
        no_attempts_today = Tracker.objects.filter(user=self.user, digest=self.digest, completed=True,
    ...
    def to_xml_string(course, user):
        ...
        trackers = Tracker.objects.filter(user=user, course=course, completed=True).values('digest').annotate(
...
class Client(models.Model):
    ...
    husband_name = models.CharField(blank=True, max_length=100, default='', null=True)
    youngest_child_age = models.IntegerField(blank=True, default=0)
    is_deleted = models.BooleanField(default=False)
    is_closed = models.BooleanField(default=False)
    adapted_method = models.CharField(blank=True, null=True, max_length=120, default='')
